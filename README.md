...
Most of what we write online disappears. A tweet scrolls away, a note that unintentionally gets deleted, a wall gets painted over. Wish Board works differently. It's a single wall, open to anyone with a wallet, where a person can write a short and honest wish, then hang it up for good. Once it's on the wall, it stays there. No account, no algorithm deciding who sees it, no owner who can quietly take it down. Just a message, a timestamp, and the address that hung it, sitting on-chain where anyone can read it.

This is built this for BOTChain Build Week. This project is intended to give people one honest place to say what they want, and let it last.
...
- **Write a wish.** Up to 280 characters, plain text, without formatting.
- **Hang it on the wall.** This sends a transaction to the WishBoard smart contract. Once it confirms, the wish is permanent. There's no delete button, by design.
